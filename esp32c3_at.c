                     ...
                     uint32_t timeout_ms)
{
    ...
    while ((uint32_t)(ctx->io.tick_ms(ctx->io.user) - start) < timeout_ms) {
        ...
        if (ctx->io.background_process != NULL) {
            ctx->io.background_process(ctx->io.user);
        }
        if (count != 0U) {
...
void esp_at_process(esp_at_t *ctx)
{
    ...
    case ESP_AT_STATE_CONFIGURE:
        if (configure(ctx)) {
            ...
            ctx->state = ESP_AT_STATE_WAIT_RESET_RETRY;
            ctx->retry_at_ms = now + 1000U;
        }
        break;
    ...
    case ESP_AT_STATE_ONLINE:
        poll_events(ctx);
        break;
    case ESP_AT_STATE_WAIT_RESET_RETRY:
