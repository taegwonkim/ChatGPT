...
* 시작할 때 `SCB->SCR`의 `SLEEPONEXIT`와 `SLEEPDEEP` 비트를 명시적으로 지웁니다.
  따라서 인터럽트 복귀 시 자동으로 Sleep에 진입하지 않으며 Deep Sleep도 선택되지
  않습니다. 메인 루프에는 WFI/WFE 명령이 없으므로 CPU는 계속 Run 모드입니다.
...
RTC Wake-up Timer와 Alarm A는 CPU가 Run 모드에서 다른 코드를 실행하는 동안에도
독립적으로 시간을 세며, 설정된 시점에 인터럽트를 발생시켜 MCU를 리셋합니다.
