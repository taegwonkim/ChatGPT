...
4. **NVIC Settings**에서 USART1 및 USART2 global interrupt를 활성화합니다. USART3는 이 예제에서 TX polling만 사용하므로 TX interrupt가 필수는 아닙니다.
5. 코드를 생성해 `usart.h/.c`에 `huart1`, `huart2`, `huart3`와 `MX_USART1/2/3_UART_Init()`가 생성되었는지 확인합니다.
6. `esp32c3_at.c/.h`, `app_wifi.c/.h`, `app_data.c/.h`를 프로젝트의 `Core/Src`, `Core/Inc`에 각각 추가하고 `app_wifi.c`의 AP/서버 설정값을 수정합니다.
