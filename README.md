# STM32L562 RTC 10-minute software reset example
...
requests a software reset every 600 seconds.
...
The `.ioc` enables RTC wake-up and its interrupt. `MX_RTC_Init()` programs the wake-up
timer for 599 + 1 one-second RTC `ck_spre` ticks. The interrupt is handled by the HAL,
...
* The first reset occurs approximately ten minutes after `MX_RTC_Init()` runs. Every
  reboot re-arms the timer, so boot time is not included in the ten-minute interval.
