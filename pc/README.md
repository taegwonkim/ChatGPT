...
반드시 MainForm의 Measurement 영역이 아니라 Solution Explorer의 **`MeasurementPanel.cs` 자체를 우클릭 → 디자이너 보기**로 여십시오. MainForm Designer에 배치된 MeasurementPanel은 하나의 UserControl이므로 부모 Form 화면에서는 내부 컨트롤을 직접 선택할 수 없습니다. Designer 연결을 안정화하기 위해 `MeasurementPanel.resx`도 `MeasurementPanel.cs`의 종속 리소스로 등록했습니다. 변경 후에도 빈 화면이면 Visual Studio를 닫고 프로젝트의 `.vs`, `bin`, `obj` 폴더를 삭제한 뒤 solution을 다시 열어 Rebuild 하십시오.
